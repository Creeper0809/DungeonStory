...
def find_project_root(start: Path) -> Path:
    for candidate in (start.parent, *start.parents):
        if (candidate / "Assets" / "Scripts").is_dir():
            return candidate
    raise RuntimeError(
        f"Could not locate a Unity project root above inventory tool: {start}"
    )
...
REVIEW_SCHEMA = "gameplay-outcome-ledger-producer-dispositions@1"
ALLOWED_COVERAGE = {"Record", "NonResult"}
ALLOWED_INTEGRATION = {
    "connected",
...
CONNECTED_PROJECTION_FIELDS = (
    "current_transaction_boundary",
    "joint_commit_feasibility",
    "outcome_save_owner",
    "existing_outbox_or_commit_evidence",
)
...
PHASE80_INFRASTRUCTURE_TYPES_OUTSIDE_NARRATIVE = {
    # Owner-facing participants/results introduced by Phase80 deliberately live
    # beside their domain authority rather than under the Narrative folder.
    "IPreparedPhysicalItemGameplayOutcome",
    "IPreparedProductionCommandOutcome",
    "ProductionCommandOutcomeCommitResult",
    "IPreparedInfrastructureCommandOutcome",
    "InfrastructureCommandOutcomeCommitResult",
    "CharacterPreparedAggregateDamageReceipt",
    # Internal adapters that return only durable-commit status for the
    # separately catalogued migrated producer receipt.  They are not a second
    # semantic gameplay result and must stay outside the pre-existing producer
    # closure denominator.
    "CharacterConsumablesOutcomeCommitResult",
    "MetaUpgradePurchaseOutcomeCommitResult",
    "MetaRunResultOutcomeCommitResult",
}
...
def load_reviewed_disposition(path: Path) -> dict[tuple[str, str], dict]:
    if path is None:
        return {}
    ...
    if payload.get("schema") != REVIEW_SCHEMA:
        raise ValueError(
            f"Disposition file must use schema {REVIEW_SCHEMA}: {resolved}"
        )
    rows = payload.get("entries")
    if not isinstance(rows, list):
        raise ValueError("Disposition file entries must be an array.")
    ...
    for index, row in enumerate(rows):
        ...
        definition_path = str(row.get("definition_path", "")).strip().replace("\\", "/")
        coverage = str(row.get("coverage_disposition", "")).strip()
        integration = str(row.get("integration_status", "")).strip()
        reason = str(row.get("reason", "")).strip()
        integration_reason = str(row.get("integration_reason", "")).strip()
        replacement = str(row.get("replacement_authority", "")).strip()
        evidence = row.get("evidence")
        if not type_name or not definition_path:
            raise ValueError(f"Disposition entry {index} requires type and definition_path.")
        if coverage not in ALLOWED_COVERAGE:
            raise ValueError(f"Disposition entry {index} has invalid coverage_disposition: {coverage}")
        if integration not in ALLOWED_INTEGRATION:
            raise ValueError(f"Disposition entry {index} has invalid integration_status: {integration}")
        if coverage == "NonResult" and integration != "excluded":
            raise ValueError(f"NonResult disposition entry {index} must be excluded.")
        if coverage == "Record" and integration == "excluded":
            raise ValueError(f"Record disposition entry {index} cannot be excluded.")
        if not reason or not integration_reason:
            raise ValueError(f"Disposition entry {index} requires both review reasons.")
        if coverage == "NonResult" and not replacement:
            raise ValueError(
                f"NonResult disposition entry {index} requires an exact replacement_authority."
            )
        if not isinstance(evidence, list) or not evidence or any(
            not isinstance(item, str) or not item.strip() for item in evidence
        ):
            raise ValueError(f"Disposition entry {index} requires non-empty source evidence.")

        # A reviewed connected record may supply the owner-specific facts that
        # lexical producer proximity cannot establish.  Do not infer a partial
        # projection: once any such fact is declared, all four must be explicit
        # and the cited commit/outbox evidence must be non-empty.  Older review
        # tables without these optional keys retain their existing behavior.
        projection_present = any(field in row for field in CONNECTED_PROJECTION_FIELDS)
        connected_projection = {}
        if projection_present:
            if coverage != "Record" or integration != "connected":
                raise ValueError(
                    f"Disposition entry {index} may project transaction facts only for a connected Record."
                )
            for field in CONNECTED_PROJECTION_FIELDS[:-1]:
                value = str(row.get(field, "")).strip()
                if not value:
                    raise ValueError(
                        f"Disposition entry {index} connected projection requires {field}."
                    )
                connected_projection[field] = value
            outbox_evidence = row.get("existing_outbox_or_commit_evidence")
            if not isinstance(outbox_evidence, list) or not outbox_evidence or any(
                not isinstance(item, str) or not item.strip()
                for item in outbox_evidence
            ):
                raise ValueError(
                    f"Disposition entry {index} connected projection requires non-empty "
                    "existing_outbox_or_commit_evidence."
                )
            connected_projection["existing_outbox_or_commit_evidence"] = [
                item.strip() for item in outbox_evidence
            ]
        key = (type_name, definition_path)
        if key in reviewed:
            raise ValueError(f"Duplicate disposition key: {type_name} @ {definition_path}")
        reviewed[key] = {
...
def load_reviewed_dispositions(
    paths: Iterable[Path] | None,
) -> tuple[dict[tuple[str, str], dict], bytes]:
    reviewed: dict[tuple[str, str], dict] = {}
    for path in paths or []:
        for key, row in load_reviewed_disposition(path).items():
            if key in reviewed:
                raise ValueError(
                    f"Duplicate disposition key across files: {key[0]} @ {key[1]}"
                )
            reviewed[key] = row
    ...
    entries = []
    for (type_name, definition_path), row in sorted(
        reviewed.items(), key=lambda item: (item[0][0].lower(), item[0][1].lower())
    ):
        entries.append(
            {
                "type": type_name,
    ...
    payload = {
        ...
        "scope": "merged-reviewed-dispositions",
        "entries": entries,
    }
    return (
        reviewed,
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        action="append",
        help=(
            "Optional fail-closed human/Codex review table keyed by type and "
            "definition path. Repeat the option to merge disjoint reviewed groups."
        ),
    )
    args = parser.parse_args()
    reviewed_dispositions, disposition_bytes = load_reviewed_dispositions(
        args.dispositions
    )
    out = args.output_dir.resolve()
    if out.exists() and any(out.iterdir()):
        raise FileExistsError(
            f"Refusing to overwrite non-empty evidence directory: {out}"
        )
    ...
    disposition_sha256 = ""
    if disposition_bytes:
        disposition_sha256 = sha256_bytes(disposition_bytes)
        (out / "reviewed-dispositions.json").write_bytes(disposition_bytes)
    files = cs_files()
                    ...
                    if identifier and identifier.group("name") in variable_types:
                        ...
                        if indexed:
                            collection = re.search(
                                rf"\b(?:IReadOnlyList|IList|ICollection|IEnumerable|List)\s*<\s*"
                                rf"(?P<type>{base_union_text})\s*>\s+{re.escape(indexed.group('name'))}\b",
                                stripped,
                            )
                        if collection:
                            event_type = collection.group("type")
                            resolution = "lexical-indexed-collection-declaration"
                        else:
    ...
    matched_dispositions: set[tuple[str, str]] = set()
    for entry in definitions:
        ...
        review_key = (entry["base_type"], entry["definition_path"])
        reviewed = reviewed_dispositions.get(review_key)
        if reviewed is not None:
            matched_dispositions.add(review_key)
            coverage_disposition = reviewed["coverage_disposition"]
            integration_status = reviewed["integration_status"]
            reason = reviewed["reason"]
            if coverage_disposition == "NonResult":
                classification = "NonResult"
                intended = "N/A"
            else:
                classification = (
                    "Record" if integration_status == "connected" else "needs-authority-fix"
                )
                intended = "recorded" if integration_status == "connected" else "planned"
        ...
        connected_projection = (
            ...
            and all(field in reviewed for field in CONNECTED_PROJECTION_FIELDS)
        )
        if connected_projection:
            boundary = reviewed["current_transaction_boundary"]
            joint_commit = reviewed["joint_commit_feasibility"]
            outcome_save_owner = reviewed["outcome_save_owner"]
            outbox_evidence = reviewed["existing_outbox_or_commit_evidence"]
        elif classification == "NonResult":
        ...
        entry.update({
            ...
            "coverage_reason": (
                reason if reviewed is not None or coverage_disposition != "Record"
                else record_coverage_reason(entry["base_type"], entry["kind"])
            ),
            "intended_ledger_status": intended,
            "integration_status": integration_status,
            "integration_reason": (
                reviewed["integration_reason"]
                if reviewed is not None
                else "No lexically proven producer exists; establish or remove the authoritative producer before integration."
            ...
            "reviewed_disposition": reviewed is not None,
            "replacement_authority": reviewed["replacement_authority"] if reviewed is not None else "",
            "review_evidence": reviewed["evidence"] if reviewed is not None else [],
            ...
            "outcome_save_owner": outcome_save_owner,
    ...
    unmatched_dispositions = sorted(set(reviewed_dispositions) - matched_dispositions)
    if unmatched_dispositions:
        formatted = ", ".join(f"{name}@{path}" for name, path in unmatched_dispositions)
        raise ValueError(f"Disposition table contains rows absent from the current source inventory: {formatted}")
    ...
    metadata = {
        ...
        "completeness_claim": (
            ...
            and sum(
                row["inventory_scope"] == "pre-existing-runtime-candidate"
                and not row["reviewed_disposition"]
                for row in definitions
            ) == 0
            else "complete-lexical-scan-review-partial-with-listed-limitations"
            if args.final_frozen and snapshot_stable
        ...
        "reviewed_disposition_count": len(reviewed_dispositions),
        "reviewed_coverage_disposition_counts": {
            value: sum(
                row["reviewed_disposition"] and row["coverage_disposition"] == value
                for row in definitions
            )
            for value in ("Record", "NonResult")
        },
        "reviewed_integration_status_counts": {
            value: sum(
                row["reviewed_disposition"] and row["integration_status"] == value
                for row in definitions
            )
            for value in ("excluded", "needs-authority-fix", "transaction-integration-pending", "connected")
        },
        "unreviewed_preexisting_definition_count": sum(
            row["inventory_scope"] == "pre-existing-runtime-candidate"
            and not row["reviewed_disposition"]
            for row in definitions
        ),
        "reviewed_disposition_source_sha256": disposition_sha256,
    ...
    csv_columns = (
        ...
        "reviewed_disposition", "replacement_authority", "review_evidence",
    ...
    with (out / "producer-inventory.csv").open("w", encoding="utf-8-sig", newline="") as handle:
        ...
        for row in definitions:
            ...
            writer.writerow({
                ...
                "replacement_authority": row["replacement_authority"],
                "review_evidence": " | ".join(row["review_evidence"]),
    ...
    review_verdict = (
        "COMPLETE"
        if metadata["unreviewed_preexisting_definition_count"] == 0
        and metadata["integration_status_counts"]["needs-authority-fix"] == 0
        and metadata["integration_status_counts"]["transaction-integration-pending"] == 0
        else "INCOMPLETE"
    )
    (out / "REVIEW.md").write_text(
        "# Gameplay Outcome Ledger Producer Inventory Review\n\n"
        f"- Verdict: `{review_verdict}`\n"
        f"- Source snapshot stable: `{snapshot_stable}`\n"
        f"- Reviewed dispositions: `{metadata['reviewed_disposition_count']}`\n"
        f"- Unreviewed pre-existing definitions: `{metadata['unreviewed_preexisting_definition_count']}`\n"
        f"- Connected records: `{metadata['integration_status_counts']['connected']}`\n"
        f"- Transaction integrations pending: `{metadata['integration_status_counts']['transaction-integration-pending']}`\n"
        f"- Authority fixes pending: `{metadata['integration_status_counts']['needs-authority-fix']}`\n\n"
        "This is a static lexical inventory plus explicit reviewed dispositions. "
        "It is not gameplay-completion evidence, and constructor/publish proximity alone "
        "does not prove atomic outcome persistence.\n",
        encoding="utf-8",
    )
    artifact_hashes = {}
    artifact_names = [
    ...
    if disposition_bytes:
        artifact_names.append("reviewed-dispositions.json")
