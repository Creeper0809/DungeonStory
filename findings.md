...
# 2026-08-13 AI lifecycle audit continuation

- The live survival authority and deterministic need calibrator had diverged: live defaults/assets author excretion `55/day` and hygiene `54/day`, while `NeedBalanceCandidate.Standard` still used `24/18` and searched only `20-30` / `15-24`. The calibrator therefore could not validate the running game.
- The five-day verifier's generic inactive-phase detector waited 15 seconds to classify the orphan `Work` action observed in seed 157182. A dedicated invariant can identify it after 2 seconds from `branch=Work`, an active executed brain action, no active `AbilityWork` routine, no work ownership, and no routine-need finalization.
- The production need-state service applies excretion from the maximum hunger/thirst multiplier and hygiene from the species hygiene multiplier. Current biological species author hygiene `1.0`; Golem authors `0.6`. The deterministic model already mirrors the excretion biological multiplier but not the Golem hygiene multiplier.
- Recoverable inspection error: PowerShell passed a Unix-style wildcard path directly to `rg`, which returned exit 1 after the requested source file had already printed. No file changed; later searches use `rg` on the parent directory with `-g` filters.

## 2026-08-13 AI executor lifecycle cross-audit

- Three independent read-only audits found that action selection is not the only source of AI lockups. Executor ownership is inconsistent across Work, safe drink, facility shopping, haul, hunt, rescue, substance use and movement.
- Proven P0 safe-drink defect: retry cooldown, per-frame admission throttling, or temporary plan failure started only a retry timer but returned success to `AIDrink`. The selected action remained started/executed without an external intent; duplicate-execution suppression then prevented it from ever retrying.
- Proven P0 Work defect: when a work target became Unity fake-null after an execution yield, the code requested a replan while the Work action was still running. The brain correctly protected that running action, after which the coroutine cleared its owners without terminating the expected action. Workwear return and worker deallocation were also missing from that branch.
- Proven P0 exception defect: `CharacterActor.TryExecuteSelectedAiAction` committed and marked the action started before calling `Execute`, with no exception boundary. One synchronous executor exception therefore left the action permanently running.
- Lifecycle cleanup was asymmetric: non-active `CharacterLifecycle` states assigned `bestAction = null` directly; actor disable/destroy only unregistered scheduler/world bridges; haul had its own `OnDisable` cleanup while Work and most other executors did not.
- The five-day verifier previously reset every stall window whenever `AbilityWork.isWorking` was true, so a task could report working forever while approved WU stayed unchanged. Need service and queue phases were also excluded from active-phase stall detection.
- The synchronous `RunAll`/`RunScaleSuite` stress helpers are not valid 500-NPC evidence because the synchronous path rejects counts above 100. Valid 500-NPC evidence must use the PlayMode profile request and normal editor loop.
- Remaining P1 cross-domain risks after the current P0 repair include all-yield `finally` coverage, stale terminal calls that can end replacement actions, facility occupancy/session cleanup, physical MealPlan authorization after action cancellation, and item lease cleanup on actor teardown. These require fault-injected PlayMode scenarios; they are not claimed fixed by the synchronous matrix.

## 2026-08-13 AI lifecycle hardening evidence

- Safe drink retry deferral now releases selected-action ownership until its authored retry timer wakes the scheduler. A temporary plan/admission failure can no longer remain a permanently started `AIDrink` action.
- Synchronous `AIActionSet.Execute` exceptions now produce an exact typed execution failure, call `OnStop` once, release reservations/movement/blackboard ownership and terminal the expected action. A focused forced-exception scenario passes.
- Non-active lifecycle transitions plus actor disable/destroy use one idempotent brain cleanup boundary instead of assigning `bestAction = null` directly. A focused Active -> Downed -> Active scenario proves cleanup and later reactivation.
- Destroyed Work targets now terminal after workwear/environment/skill/worker owners are released. Suppression runs carry a work-run generation and expected-action predicate, so action replacement stops later movement, damage and terminal writes.
- The five-day verifier now treats `isWorking` without approved-WU revision, need queues without heartbeat, and overlong need service as harmful stalls. The previous blanket `isWorking` reset no longer hides zero-progress work.
