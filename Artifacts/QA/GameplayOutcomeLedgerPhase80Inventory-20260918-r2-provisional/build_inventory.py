...
def domain_group(name: str, path: str) -> tuple[int, str]:
    if is_phase80_infrastructure(path):
...
def classify(name: str, kind: str, path: str, producer_count: int) -> tuple[str, str, str, str, str]:
    lowered = path.lower()
    if is_phase80_infrastructure(lowered):
        return (
            "NonResult",
...
def is_phase80_infrastructure(path: str) -> bool:
    ...
    for entry in definitions:
        ...
        entry.update({
            "inventory_scope": "phase80-implementation-infrastructure"
                if is_phase80_infrastructure(entry["definition_path"])
