...
이 디렉터리의 기준 진입점은 [현행 게임 시스템 설계 핸드북](handbook/README.md)이다. 핸드북은 2026-08-30의 코드와 작성 자산을 근거로 현재 시스템의 구조를 설명하고, 종합 설계 문서에 기록된 제작 의도와 플레이 경험을 함께 편찬한다. 구현 여부와 후속 과제는 [시스템 구현 권위 체크리스트](system-implementation-checklist.md)에서 관리한다.
...
플레이어용 공식 위키의 구현 전 제품·정보·기술·운영 기준은 [DungeonStory 플레이어 위키 구축 계획](wiki/README.md)에서 관리한다. 위키는 `docs_final`을 읽기 전용 입력으로 사용하며, 사이트 애플리케이션은 승인 후 저장소 루트 `wiki/`에 분리한다.
...
4. [시스템 구성과 상태 권위](handbook/02-system-map-and-authority.md)
5. [코드 아키텍처 문서군](architecture/code-architecture-guide.md)
6. [전체 런타임 구조](architecture/08-whole-runtime-topology.md)
7. [상태 권위 원장](architecture/09-state-authority-ledger.md)
8. [도메인 간 거래와 실패 경계](architecture/10-cross-domain-transactions.md)
9. [런타임 스케줄링과 읽기 투영](architecture/11-runtime-scheduling-and-projections.md)
10. [시스템 구현과 성능 설계](architecture/systems/README.md)
11. [콘텐츠 데이터베이스](content-db/README.md)
12. 관심 있는 시스템 장
13. [시스템 구현 권위 체크리스트](system-implementation-checklist.md)
...
| 5 | [콘텐츠 데이터베이스](content-db/README.md) | 작성 자산별 규칙, 연결 관계, 존재 이유와 구현 권위 |
| 6 | `handbook/` | 사람이 읽는 현행 시스템 설명 |
| 7 | 신규 목표 설계 문서 | 아직 구현되지 않은 계약과 콘텐츠 이행 범위 |
| 8 | `docs/generated/`, `docs/implementation-reports/`, `docs/qa/` | 특정 시점의 생성 결과와 검증 증거 |
| 9 | 나머지 기존 설계 문서 | 상세 아이디어와 역사적 맥락 |
...
| 종류 | 수 | 집계 기준 |
|---|---:|---|
| 연구 프로젝트 | 180 | 현재 작성된 연구 프로젝트 |
| 아이템 정의 | 1,074 | 실제 품목을 나타내는 아이템 자산 |
| 건물 정의 | 419 | 현재 작성된 건물 자산 |
| 실제 사용 대상 건물 | 398 | 구형 호환용 자산 21개 제외 |
| 작물 정의 | 12 | 현재 작성된 작물 자산 |
| 야생동물 종 | 18 | 현재 작성된 야생동물 종 |
| 질병 | 16 | 현재 작성된 질병 자산 |
| 공격 조우 | 36 | 현재 작성된 공격 조우 |
| 던전 세력 | 12 | 현재 작성된 던전 세력 |
| 시설 합성 조합식 | 9 | 현재 작성된 시설 합성 조합식 |
| 시설 진화 조합식 | 6 | 현재 작성된 시설 진화 조합식 |

이 수치는 현재 작업 트리에서 집계한 값이다. 콘텐츠를 추가하거나 이관하면 다시 집계해야 한다.

개별 아이템·시설·생산식·특성·사건·연구·전투·의료·세계 콘텐츠의 안정 식별자, 핵심 규칙, 참조 관계와 존재 이유는 [콘텐츠 데이터베이스](content-db/README.md)에서 조회한다. 전체 3,475개 작성 항목과 정리된 연결 관계 5,667개는 현재 자산에서 직접 추출한다.
