...
### V27 물리 BOM 수치 반영 및 검증 상태 — 2026-08-31

- `bedding-straw`, `cloth`, 조리·보존·약초·유제품·원정 식량 16개 Transform의 실제 BOM, clean water, wastewater, 잔차, WU, 산출 kg, 단가를 `content-plans/production/production-physical-bom-ledger.csv`에 기록했다.
- GameContentCatalog가 참조하는 weighted item `1,074`개와 Transform `328`개를 정적으로 재계산했다. unknown reference `0`, Transform 질량 생성 `0`, 대상 descriptor `16/16`, 잔차 `30–200g`이다.
- 이 batch는 ItemSO unit price, output count, item kg, recipe WU, 시설·연구·소비자·저장 schema를 바꾸지 않았다. 새 BOM이 embedded work와 시장 가격·보상에 주는 영향은 재계산 대상으로 남아 있다.
- `Artifacts/QA/v27-balance-economy-256-seed.txt`는 paired-run 증거가 이전 소스여서 현재 FAIL이다. market authority, labor/facility authority, 6인 생존 loop, consumer/restore, output capacity도 새 BOM 기준 실행 결과가 없다.
- 전투는 `CombatBalanceCheckpointAuthority`의 36개 조우 파라미터와 보관된 final checkpoint 36개가 모두 일치한다. current-source 전투 재실행은 아직 없다. 예전 `combat-outcome-balance.txt`의 FAIL은 현재 조우 권위값과 다른 실행 결과이며 새 기준으로 사용하지 않는다.
- 빌드 간 지배성, 막힌 빌드, 사건 선택의 결과 차이는 아직 측정되지 않았다. 완료 조건과 실행 상태는 `balance-validation-status.md`가 권위다.
...
- 검증 매트릭스와 보고서: 순수 규칙 EditMode, 전환 실패 복원, 포로 V3 이관 및 새 저장 왕복, 경비/원정 권한, 임금 0·생활 소비 100%·XP 50%·멘토링 차단, UI 표면, 256시드 운영 감사를 요구한다. 결과는 `Artifacts/QA/minion-resident-standing-audit.txt`에 남긴다.
...
### `balance:narrative-manual-work-skill-targeting:v1` — 2026-09-16

- 정의 ID·종류: 서사 공식이 만든 캐릭터 액티브 중 관리 capability를 플레이어가 직접 사용하는 임시 작업 능력으로 연결한다. 자동 `WorkStarted` 패시브나 전투 턴 기술로 취급하지 않는다.
- 플레이어 결정: 캐릭터 능력 버튼에서 `자신`, `선택`, `결정적 무작위`, `전체` 대상 획득을 사용한다. 선택형은 합법 캐릭터를 직접 고르며, 방·N×N 범위에서는 시설도 중심점으로 고를 수 있다.
- 시간: 기본 공식 결과는 게임 시간 24시간 지속, 사용 후 2일(48시간) 재사용 대기다. 작업 능력에는 전투 턴을 기록하지 않는다.
- 공간: `단일`, `같은 방`, 선택 중심 `3×3/5×5/7×7`, `던전 전체`를 지원한다. 대상 획득과 효과 범위는 별도 필드이며 자신은 단일, 전체는 던전 전체 조합만 합법이다.
- 비용: 범위는 공식의 대상 수 비용 문맥에 포함한다. 단일 1, 방 3, N×N은 폭 N, 던전 전체 8의 보수적 동시 대상 추정치를 사용하며 실제 적용 대상은 현재 합법 후보로 다시 필터링한다.
- 부정 효과 크레딧: 필수 2일 재사용 대기는 해당 액티브 인스턴스의 선택형 부담으로만 크레딧을 준다. 다른 기술·특성·시설로 예산을 이전하지 않는다.
- 실행·저장 권위: C#이 합법 대상, 방/격자 범위, 고정 시드 무작위, 24시간 만료와 48시간 준비 시점을 계산한다. 대상별 동결 기술과 시전자별 준비 시각·사용 횟수를 캐릭터 성장 상태에 저장한다. LLM은 이름과 서사 문장만 작성한다.
- 위험·실패: 짝수 N, 3 미만·7 초과 N, 자신+광역, 전체+비던전, 범위 안 합법 대상 없음, 그리드·방 조회 없음과 미등록 적용기는 fail-closed다. 시간에 따라 무작위 대상이 바뀌지 않도록 시드는 캐릭터 ID·기술 ID·사용 횟수·공식 버전·카탈로그 해시에 결속한다.
- 검증: `CharacterProgressionDebugScenarios.RunManualWorkSkillTargetingScenario`가 네 대상 획득 방식, 방·3×3, 24시간 만료, 48시간 재사용 대기, 저장 clone과 시간 독립 결정적 무작위를 검사한다.
- 현재 판정: 런타임·UI·저장·검수 자연어 연결 구현 및 focused 시나리오 통과. 대규모 밸런스 시뮬레이션과 새 100행 사람 검수 전에는 학습·출시 승격 근거로 사용하지 않는다.
