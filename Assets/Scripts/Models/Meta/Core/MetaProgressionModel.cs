...
internal sealed class MetaProgressionAggregateState
{
    internal Dictionary<string, int> UpgradeLevels { get; } =
        new(System.StringComparer.Ordinal);
    internal HashSet<string> PreservedRecipeIds { get; } = new();
    internal IReadOnlyDictionary<string, int> UpgradeLevelsView { get; }
    internal IReadOnlyCollection<string> PreservedRecipeIdsView { get; }
    internal int LifetimeEarnedCurrency { get; set; }
    internal int SpentCurrency { get; set; }
    internal int CompletedRunCount { get; set; }

    internal MetaProgressionAggregateState()
    {
...
public sealed class MetaProgressionState
{
    ...
    private MetaProgressionAggregateState state
    {
        get => aggregateRootStore.GetOrCreate(
            () => new MetaProgressionAggregateState());
        set => aggregateRootStore.Replace(value);
    }

    private MetaProgressionAggregateState writableState =>
        aggregateRootStore.GetOrCreateWritable(
            () => new MetaProgressionAggregateState(),
            CloneState);

    private Dictionary<string, int> upgradeLevels => writableState.UpgradeLevels;
    private HashSet<string> preservedRecipeIds => writableState.PreservedRecipeIds;

    public MetaProgressionState(
        IMetaUpgradeDefinitionCatalog catalog,
        DungeonRuntimeAggregateRootStore aggregateRootStore)
    {
        Catalog = catalog ?? throw new System.ArgumentNullException(nameof(catalog));
        this.aggregateRootStore = aggregateRootStore
            ?? throw new System.ArgumentNullException(nameof(aggregateRootStore));
    }

    public int LifetimeEarnedCurrency
    {
        get => state.LifetimeEarnedCurrency;
        private set => writableState.LifetimeEarnedCurrency = value;
    }
    public IMetaUpgradeDefinitionCatalog Catalog { get; }
    public int SpentCurrency
    {
        get => state.SpentCurrency;
        private set => writableState.SpentCurrency = value;
    }
    public int CompletedRunCount
    {
        get => state.CompletedRunCount;
        private set => writableState.CompletedRunCount = value;
    }
    public int AvailableCurrency => Mathf.Max(0, LifetimeEarnedCurrency - SpentCurrency);
    public IReadOnlyDictionary<string, int> UpgradeLevels => state.UpgradeLevelsView;
    public IReadOnlyCollection<string> PreservedRecipeIds =>
        state.PreservedRecipeIdsView;
    ...
    {
        MetaUpgradeDefinition definition = Catalog.Get(id);
        if (definition == null)
        {
            message = "강화 정보가 없습니다";
            return false;
    ...
    {
        int restoredEarnedCurrency = Mathf.Max(0, lifetimeEarnedCurrency);
        MetaProgressionAggregateState restored = new()
        {
            LifetimeEarnedCurrency = restoredEarnedCurrency,
            SpentCurrency = Mathf.Clamp(
                spentCurrency,
                0,
                restoredEarnedCurrency),
            ...
            ?? System.Array.Empty<KeyValuePair<string, int>>())
        {
            ...
            {
                MetaUpgradeDefinition definition = Catalog.Get(pair.Key);
                int maximum = definition != null ? Mathf.Max(1, definition.maxLevel) : pair.Value;
                restored.UpgradeLevels[pair.Key] = Mathf.Clamp(
                    pair.Value,
                    0,
                    maximum);
            }
        ...
        foreach (string recipeId in savedRecipeIds ?? System.Array.Empty<string>())
        {
            ...
            {
                restored.PreservedRecipeIds.Add(recipeId.Trim());
            }
        }
    ...
    private static MetaProgressionAggregateState CloneState(
        MetaProgressionAggregateState source)
    {
        MetaProgressionAggregateState clone = new()
        {
            LifetimeEarnedCurrency = source?.LifetimeEarnedCurrency ?? 0,
            SpentCurrency = source?.SpentCurrency ?? 0,
            CompletedRunCount = source?.CompletedRunCount ?? 0
        };
        if (source == null)
        {
            return clone;
        ...
        foreach (KeyValuePair<string, int> pair in source.UpgradeLevels)
        {
            clone.UpgradeLevels.Add(pair.Key, pair.Value);
        }

        foreach (string recipeId in source.PreservedRecipeIds)
        {
            clone.PreservedRecipeIds.Add(recipeId);
        }
