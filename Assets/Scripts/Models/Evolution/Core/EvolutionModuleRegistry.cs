...
public sealed class EvolutionModuleRegistry : IEvolutionModuleRegistry
{
    ...
    private static IEnumerable<EvolutionModuleDefinition> CreateBuiltIns()
    {
        ...
        yield return Module("equipment:force", "위력의 계보", "force",
            Benefit("combat.damage", 0f, 1.08f), Burden("combat.weight", 0.08f));
        ...
        yield return Module("equipment:precision", "정밀의 계보", "precision",
            Benefit("combat.accuracy", 0f, 1.08f), Burden("combat.damage", 0f, 0.98f));
        yield return Module("equipment:cadence", "속도의 계보", "cadence",
            Benefit("combat.reload", 0f, 0.92f), Burden("combat.durability", 0f, 0.96f));
        yield return Module("equipment:control", "제압의 계보", "control",
            Benefit("combat.accuracy", 0f, 1.05f), Burden("combat.weight", 0.05f));
        yield return Module("equipment:execution", "처형의 계보", "execution",
            Benefit("combat.damage", 0f, 1.1f), Burden("combat.value", 0f, 1.08f));
        yield return Module("equipment:durability", "내구의 계보", "durability",
            Benefit("combat.durability", 0f, 1.12f), Burden("combat.weight", 0.06f));
        yield return Module(
            "equipment:risky",
            "불안정 각인",
            "risk",
