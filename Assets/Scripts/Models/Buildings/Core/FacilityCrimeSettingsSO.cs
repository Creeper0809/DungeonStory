...
public sealed class ResourceFacilityCrimeSettingsProvider : IFacilityCrimeSettingsProvider
{
    ...
    private readonly FacilityCrimeSettingsSO settings;

    public ResourceFacilityCrimeSettingsProvider(IGameContentDefinitionSource content)
    {
        settings = (content ?? throw new ArgumentNullException(nameof(content)))
            .RequireSingle<FacilityCrimeSettingsSO>();
    }

    public FacilityCrimeSettingsSO Settings => settings;
}
