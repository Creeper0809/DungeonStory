...
public static class V20FactionServiceContentAssetBuilder
{
    ...
    {
        S("beastkin","faction:dungeon:beastkin","붉은발톱 교역소","사냥터와 피난민 사이에서 무리의 생존 규칙을 다시 정하려 한다.","faction:dungeon:demon","food:salted-meat-stew","medicine:standard","equipment-item:shield:tower",
        ...
        S("demon","faction:dungeon:demon","잿빛 계약정","불완전한 계약과 계승 분쟁을 이용해 세력을 재편하려 한다.","faction:dungeon:golem","material:paper","component:rune-conductor","tool:administrative-seal",
        ...
        S("golem","faction:dungeon:golem","돌맥 주조소","마모된 핵과 생산 할당량 사이에서 인격을 가진 구성체의 권리를 묻는다.","faction:dungeon:kobold","component:machine-parts","tool:maintenance-kit","component:rune-conductor",
        ...
        S("harpy","faction:dungeon:harpy","폭풍둥지","하늘길과 노래 기록의 독점을 둘러싸고 둥지 내부가 갈라졌다.","faction:dungeon:myconid","material:rope","medicine:antiseptic","tool:weather-observation-kit",
        ...
        S("kobold","faction:dungeon:kobold","깊은톱니 굴","도구와 설계의 소유를 씨족이 가질지 제작자가 가질지 다툰다.","faction:dungeon:harpy","component:machine-parts","tool:field-repair-kit","component:engineering-drawing",
        ...
        S("myconid","faction:dungeon:myconid","균사정원","공동 기억망이 병든 포자와 오래된 조상의 의지를 함께 퍼뜨린다.","faction:dungeon:beastkin","resource:cave-mushroom","supply:fungicide","tool:weather-observation-kit",
    ...
    private static FactionChapterDefinitionSO CreateChapter(FactionSpec spec,int number)
    {
        ...
        string itemId=number switch
        {
            ...
            3 when string.Equals(spec.Key,"demon",StringComparison.Ordinal)=>spec.CrisisItem,
    ...
    private static GuestRequestDefinitionSO CreateRequest(
        RequestSpec spec,
        ItemDefinitionCatalogSO itemCatalog)
    {
        ItemDefinitionSO item = itemCatalog.Definitions.FirstOrDefault(value =>
            value != null
            && string.Equals(value.ItemId, spec.Item, StringComparison.Ordinal))
            ?? throw new InvalidOperationException(
                $"Guest request '{spec.Id}' item '{spec.Item}' is missing.");
        int internalValue = item.UnitPrice * spec.Amount;
    ...
    private static int ContractAmount(string factionKey,int index) =>
        (factionKey,index) switch
        {
            ("beastkin",0)=>8, ("beastkin",1)=>3, ("beastkin",2)=>4,
            ("demon",0)=>8, ("demon",1)=>1, ("demon",2)=>18,
            ("golem",0)=>2, ("golem",1)=>1, ("golem",2)=>8,
            ("harpy",0)=>8, ("harpy",1)=>6, ("harpy",2)=>6,
            ("kobold",0)=>2, ("kobold",1)=>1, ("kobold",2)=>18,
            ("myconid",0)=>16, ("myconid",1)=>4, ("myconid",2)=>6,
            _=>throw new ArgumentOutOfRangeException(nameof(factionKey),$"Unknown faction contract amount: {factionKey}/{index}.")
        };
