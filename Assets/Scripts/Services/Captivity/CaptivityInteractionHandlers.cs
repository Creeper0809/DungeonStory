...
public abstract class CaptivityInteractionHandlerBase : ICaptivityInteractionHandler
{
        ...
        out string failureReason)
    {
        ...
        if (context.Subject == null || context.Subject.IsDead)
        {
            failureReason = "포로를 찾을 수 없습니다.";
            return false;
        }

        if (context.Warden == null
            || context.Warden.IsDead
            || context.Warden.CurrentLifecycleState != CharacterLifecycleState.Active)
        {
            failureReason = "담당자가 작업할 수 없는 상태입니다.";
            return false;
        }

        if (context.Facility?.BuildingData.GetCaptiveHousingAbility()?.IsValid != true)
        {
