...
public sealed class ResearchBlueprintArchiveQuery : IResearchBlueprintArchiveQuery
{
    ...
    public static string GetDestinationId(BuildableObject archive)
    {
        ...
        return $"research-archive:{archive.BuildingData?.id ?? archive.id}:{archive.centerPos.x}:{archive.centerPos.y}";
    }
...
public static class ResearchBlueprintItemDefinitions
{
    private const string Prefix = "research-blueprint:";
        ...
        out DungeonItemDefinition definition)
    {
        ...
        if (!normalized.StartsWith(Prefix, StringComparison.Ordinal)
            || !int.TryParse(normalized.Substring(Prefix.Length), out int blueprintId))
        {
            definition = null;
            return false;
        }

        FacilityBlueprintSO blueprint = facilityCatalog?.Blueprints
            .FirstOrDefault(candidate => candidate != null && candidate.id == blueprintId);
        string displayName = blueprint != null
            ? $"{blueprint.DisplayName} 설계도"
            : $"설계도 {blueprintId}";
        definition = new DungeonItemDefinition(
            normalized,
            displayName,
            blueprint?.description ?? "연구실 보관대에 배치하면 연구 조건을 충족합니다.",
            StockCategory.Blueprint,
            Mathf.Max(1, blueprint?.defaultCost ?? 80),
            null,
            0.15f,
            1);
        return true;
