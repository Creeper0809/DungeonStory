...
internal static class V27ConstructionRedistributionPolicy
{
        ...
        IReadOnlyDictionary<string, V27ItemValue> itemValues)
    {
        ...
        decimal periodLaborShare = (periodWu * 1000m) / targetInvestment;
        ...
        if (periodLaborShare <= MaterialDominatedLaborShareMaximum)
        {
            return CreatePeriodResult(
                stableId,
                beforeWu,
        ...
        void Evaluate(long wu, long bomMilli)
        {
            ...
            {
                int added = amounts[index] - options[index].BeforeAmount;
                if (added > 0)
                    changedRows++;
    ...
    {
        public MaterialOption(string itemId, int beforeAmount, int maximumAmount, long unitMilliEwu)
        {
            ItemId = itemId;
    ...
    private sealed class Candidate : IComparable<Candidate>
    {
        ...
        public int CompareTo(Candidate other)
        {
            ...
            int comparison = DispositionRank.CompareTo(other.DispositionRank);
