...
public abstract class AIDeprivationBreakdownAction : AIActionSet
{
    ...
    public override bool CanStart(CharacterActor actor)
    {
        DungeonStory.AI.AiCharacterDecisionSnapshot snapshot = new(
            ...
            actor != null,
            hasDeprivationBreakdown: actor != null
                && actor.DeprivationQuery?.HasBreakdownKind(
                    actor,
                    BreakdownKind) == true);
        return DungeonStory.AI.AIDeprivationBreakdowns.Evaluate(
            snapshot,
            (int)BreakdownKind).Allowed;
    }
    ...
    public override void Execute(CharacterActor actor)
    {
        ...
        DungeonStory.AI.AiActionRequest request =
            DungeonStory.AI.AIDeprivationBreakdowns.CreateRequest(
                snapshot,
                (int)BreakdownKind);
        if (request.IsValid)
        {
            actor?.DeprivationCommands?.BeginBreakdownAction(
                actor,
                BreakdownKind);
        }
    }
