...
public static class OffenseV17PlayModeVerifier
{
    public const string OutputDirectory = "Temp/OffenseV17Validation";
    public const string ReportPath =
        OutputDirectory + "/offense-v17-visual-report.txt";

    [MenuItem("Tools/DungeonStory/Validation/Capture Offense V17 UI")]
    public static void RunFromMenu()
    {
        if (!EditorApplication.isPlaying)
        {
            Debug.LogError("Offense V17 UI capture requires PlayMode.");
            return;
        }

        if (UnityEngine.Object.FindFirstObjectByType<
                OffenseV17VisualVerificationRunner>() != null)
        {
            Debug.LogWarning("Offense V17 UI capture is already running.");
            return;
        }

        EditorApplication.ExecuteMenuItem("Window/General/Game");
        new GameObject("Offense V17 Visual Verification Runner")
            .AddComponent<OffenseV17VisualVerificationRunner>();
    }
}

public sealed class OffenseV17VisualVerificationRunner : MonoBehaviour
{
    ...
    {
        Directory.CreateDirectory(OffenseV17PlayModeVerifier.OutputDirectory);
        ...
        string surface = ResolveV17SurfaceName();
        ...
        string summary = failures.Count == 0
            ? $"PASS Offense V17 visual verification ({surface})"
            : $"FAIL Offense V17 visual verification ({failures.Count})";
        report.Insert(0, summary);
        File.WriteAllLines(OffenseV17PlayModeVerifier.ReportPath, report);
        if (failures.Count == 0)
        {
            Debug.Log(summary);
        }
    ...
    private IEnumerator Capture(string surface, Vector2Int resolution)
    {
        ...
        string path = Path.Combine(
            OffenseV17PlayModeVerifier.OutputDirectory,
    ...
    private static string ResolveV17SurfaceName()
    {
