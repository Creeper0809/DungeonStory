...
public static class V20CombatContentAssetBuilder
{
    ...
    {
        E("enemy:crown-levy","왕령 징집병","수적 압박으로 전열을 붙잡는 징집 보병.","human:crown","Human",EnemyCombatRole.Vanguard,"line","weapon:spear","armor:padded","",82,7,6,5,4,new[]{"enemy-ability:charge"},new[]{"counter:reach"}),
        ...
        E("enemy:crown-longbow","왕령 장궁병","가장 약한 후열을 골라 제압 사격한다.","human:crown","Human",EnemyCombatRole.Marksman,"ranged-line","weapon:longbow","armor:padded","ammo:arrow",72,10,4,9,4.5f,new[]{"enemy-ability:suppressive-volley"},new[]{"counter:smoke"}),
        E("enemy:crown-chaplain","왕령 군종사제","부상병을 치료하고 전열 붕괴를 늦춘다.","human:crown","Human",EnemyCombatRole.Support,"protected-center","weapon:staff","armor:chain","",86,6,7,7,4,new[]{"enemy-ability:field-dressing","enemy-ability:rally"},new[]{"counter:focus-support"}),
        E("enemy:crown-marshal","왕령 원수","보호와 재집결을 반복하는 지휘관.","human:crown","Human",EnemyCombatRole.Boss,"command-center","weapon:longsword","armor:plate","",150,12,12,9,4.5f,new[]{"enemy-ability:rally","enemy-ability:armor-break"},new[]{"counter:isolate-leader"}),
        ...
        E("enemy:legion-pavise","도시연맹 파비스병","사격수를 가리는 이동 엄폐를 만든다.","human:legion","Human",EnemyCombatRole.Defender,"pavise-line","weapon:falchion","armor:scale","",118,7,12,4,3,new[]{"enemy-ability:shield-wall","enemy-ability:retreat-cover"},new[]{"counter:flank"}),
        E("enemy:legion-arquebus","도시연맹 아쿼버스병","갑주 목표에게 화약 일제를 집중한다.","human:legion","Human",EnemyCombatRole.Marksman,"powder-line","weapon:arquebus","armor:padded","ammo:paper-cartridge",76,12,4,8,3.5f,new[]{"enemy-ability:powder-shot","enemy-ability:smoke-screen"},new[]{"counter:rush"}),
        E("enemy:legion-surgeon","도시연맹 외과의","엄폐 뒤에서 중상자를 우선 치료한다.","human:legion","Human",EnemyCombatRole.Support,"rear-clinic","weapon:dagger","armor:padded","",78,5,5,9,4,new[]{"enemy-ability:field-dressing","enemy-ability:smoke-screen"},new[]{"counter:interrupt-heal"}),
        E("enemy:legion-captain","도시연맹 백인대장","창벽과 화약병의 사격 주기를 조율한다.","human:legion","Human",EnemyCombatRole.Controller,"command-flank","weapon:halberd","armor:plate","",132,11,10,9,4.5f,new[]{"enemy-ability:rally","enemy-ability:suppressive-volley"},new[]{"counter:break-formation"}),
        ...
        E("enemy:inquisition-ward-knight","성화 결계기사","마법 공격을 막는 룬 방패로 심문관을 보호한다.","human:inquisition","Human",EnemyCombatRole.Defender,"ward-circle","weapon:mace","armor:plate","",126,8,13,5,3.5f,new[]{"enemy-ability:rune-ward","enemy-ability:shield-wall"},new[]{"counter:physical-burst"}),
        ...
        E("enemy:inquisition-censor","성화 검열관","아군 결계를 유지하고 적의 능력을 봉쇄한다.","human:inquisition","Human",EnemyCombatRole.Controller,"ward-center","weapon:rune-staff","armor:rune-chain","",92,8,8,8,4,new[]{"enemy-ability:arcane-null","enemy-ability:rune-ward"},new[]{"counter:dispel"}),
        E("enemy:inquisition-high-judge","성화 대심문관","전장의 최고 전력을 봉인한 뒤 정화병을 집중시킨다.","human:inquisition","Human",EnemyCombatRole.Boss,"judgement-center","weapon:rune-sword","armor:plate","",158,13,12,11,4.5f,new[]{"enemy-ability:truth-seal","enemy-ability:rally"},new[]{"counter:split-pressure"}),
        ...
        E("enemy:merchant-hired-bulwark","금화동맹 고용방벽","계약 대상과 화물을 최우선으로 보호한다.","human:merchant","Human",EnemyCombatRole.Defender,"cargo-ring","weapon:mace","armor:scale","",116,7,12,5,3.5f,new[]{"enemy-ability:shield-wall","enemy-ability:retreat-cover"},new[]{"counter:separate-cargo"}),
        ...
        E("enemy:merchant-factor","금화동맹 전투지배인","불리하면 엄호 퇴각하고 유리하면 용병을 부른다.","human:merchant","Human",EnemyCombatRole.Support,"command-rear","weapon:handgun","armor:brigandine","ammo:paper-cartridge",104,9,8,9,4,new[]{"enemy-ability:summon-minion","enemy-ability:retreat-cover"},new[]{"counter:deny-retreat"}),
        ...
        E("enemy:settler-barricadier","자유개척 바리케이드병","임시 방벽 뒤에서 약한 동료를 지킨다.","human:settler","Human",EnemyCombatRole.Defender,"scattered-cover","weapon:spear","armor:padded","",102,7,10,6,4,new[]{"enemy-ability:shield-wall"},new[]{"counter:destroy-cover"}),
        ...
        E("enemy:rival-packbreaker","경쟁 던전 무리파쇄자","갈고리로 후열을 끌어내 수인 돌격대에 넘긴다.","rival:dungeon","Beastkin",EnemyCombatRole.Vanguard,"pack-wedge","weapon:halberd","armor:scale","",124,12,9,10,5,new[]{"enemy-ability:hook-pull","enemy-ability:charge"},new[]{"counter:guard-backline"}),
        E("enemy:rival-pactbinder","경쟁 던전 계약결속사","소환체와 보호막으로 시간을 번다.","rival:dungeon","Demon",EnemyCombatRole.Controller,"pact-circle","weapon:rune-staff","armor:rune-chain","",110,10,8,11,4.5f,new[]{"enemy-ability:summon-minion","enemy-ability:rune-ward"},new[]{"counter:dispel"}),
        ...
        E("enemy:rival-spore-shepherd","경쟁 던전 포자목자","독성 포자와 소형 균사체로 지역을 장악한다.","rival:dungeon","Myconid",EnemyCombatRole.Support,"spore-garden","weapon:staff","armor:rune-chain","",118,9,10,9,3.5f,new[]{"enemy-ability:poison-cloud","enemy-ability:summon-minion"},new[]{"counter:fungicide"}),
        E("enemy:truth-sealkeeper","진실 봉인수호자","강화된 목표를 찾아 진실 봉인을 새긴다.","truth:guardian","Truth",EnemyCombatRole.Controller,"seal-triangle","weapon:rune-sword","armor:rune-chain","",142,13,12,11,4.5f,new[]{"enemy-ability:truth-seal","enemy-ability:arcane-null"},new[]{"counter:mundane"}),
        ...
        E("enemy:truth-archivist","진실 기록집행자","전투 기록에 따라 반복 전술을 봉쇄하는 보스.","truth:guardian","Truth",EnemyCombatRole.Boss,"archive-center","weapon:rune-staff","armor:powered-plate","",230,16,17,14,4,new[]{"enemy-ability:truth-seal","enemy-ability:summon-minion","enemy-ability:rally"},new[]{"counter:mixed-tactics"}),
        ...
        E("enemy:neutral-clockwork","유랑 태엽구성체","가장 가까운 장치를 보호하고 손상 시 수리한다.","neutral:construct","Golem",EnemyCombatRole.Defender,"device-guard","weapon:warhammer","armor:plate","",148,10,14,6,3.5f,new[]{"enemy-ability:shield-wall","enemy-ability:core-repair"},new[]{"counter:precision"}),
        E("enemy:neutral-smoke-sapper","연막 공병","연막 속에서 시설 목표에 접근해 파괴한다.","neutral:specialist","Human",EnemyCombatRole.Controller,"sapper-wedge","weapon:handgun","armor:blast-coat","ammo:paper-cartridge",94,10,7,11,4.5f,new[]{"enemy-ability:smoke-screen","enemy-ability:powder-shot"},new[]{"counter:detector"})
    };
    ...
    private static OffenseEncounterSO CreateEncounter(int index,IReadOnlyList<EnemyArchetypeDefinitionSO> enemies,IReadOnlyList<BattlefieldModifierDefinitionSO> modifiers)
    {
        string path=index<=6?$"Assets/Resources/SO/Offense/Encounters/encounter_{index:00}.asset":$"{Root}/Encounters/encounter_{index:00}.asset";OffenseEncounterSO v=Asset<OffenseEncounterSO>(path);v.id=index<=6?index:8000+index;v.encounterId=$"encounter:{index:00}";
        string[] names={"왕령 농지 징발","왕령 호송대","도시연맹 무기고","성화 비전 봉쇄","경쟁 던전 척후","진실 봉인실"};v.displayName=index<=6?names[index-1]:$"{enemies[index-1].displayName} 전술 조우";v.minimumSiteStrength=Mathf.Max(1,(index-1)/4+1);v.maximumSiteStrength=v.minimumSiteStrength+3;v.elite=index%5==0;v.boss=index%6==0;
        v.objective=(OffenseEncounterObjective)((index-1)%6);v.objectiveRoundLimit=v.objective==OffenseEncounterObjective.DefeatAll?0:6+(index%3);v.objectiveTargetId=v.objective switch{OffenseEncounterObjective.ProtectTarget=>"target:field-medic",OffenseEncounterObjective.SabotageTarget=>"target:war-device",OffenseEncounterObjective.CaptureLeader=>enemies[index-1].stableId,_=>string.Empty};
        v.battlefieldModifierIds=new List<string>{modifiers[(index-1)%modifiers.Count].stableId};v.counterTags=enemies[index-1].counterTags.ToList();v.rewardItemIds=enemies[index-1].rewardItemIds.ToList();v.enemies=new List<OffenseEnemyArchetypeEntry>{new(){enemyArchetypeId=enemies[index-1].stableId,minimumCount=1,maximumCount=v.boss?1:2},new(){enemyArchetypeId=enemies[(index+6)%enemies.Count].stableId,minimumCount=1,maximumCount=2}};Dirty(v);return v;
    }
