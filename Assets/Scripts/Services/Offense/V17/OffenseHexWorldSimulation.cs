...
public interface IOffenseWorldSimulation
{
    ...
    OffenseV17SaveData Capture();
    void Restore(OffenseV17SaveData saveData);
}
    ...
    IDisposable
{
    ...
    private readonly Dictionary<OffenseHexCoord, OffenseHexTileState> tiles =
        new Dictionary<OffenseHexCoord, OffenseHexTileState>();
    private readonly Dictionary<string, OffenseWorldSiteStateData> sites =
        new Dictionary<string, OffenseWorldSiteStateData>(StringComparer.Ordinal);
    private readonly Dictionary<string, OffenseUrgentSiteStateData> urgentSites =
    ...
    public OffenseHexWorldSimulation(
        IRunVariableRuntimeProvider runVariables,
    ...
    public void Start()
    {
        int seed = runVariables.TryGetRuntime(out RunVariableRuntime runtime)
            ? runtime.RunSeed
            : 1;
        if (WorldSeed == 0)
        {
    ...
    public OffenseV17SaveData Capture()
    {
        return new OffenseV17SaveData
        {
            worldSeed = WorldSeed,
            worldDay = WorldDay,
            worldHour = WorldHour,
    ...
    public void Restore(OffenseV17SaveData saveData)
    {
        if (saveData == null)
        {
        ...
        WorldSeed = saveData.worldSeed == 0 ? 1 : saveData.worldSeed;
        WorldDay = Mathf.Max(1, saveData.worldDay);
        WorldHour = Mathf.Clamp(saveData.worldHour, 0f, 24f);
        tiles.Clear();
        sites.Clear();
        urgentSites.Clear();

        foreach (OffenseHexTileState tile in saveData.tiles
                     ?? new List<OffenseHexTileState>())
        {
            if (tile == null || tiles.ContainsKey(tile.Coord))
            {
                throw new InvalidOperationException(
                    $"Duplicate or null offense world tile '{tile?.Coord.ToString() ?? "null"}'.");
            }

            tiles.Add(tile.Coord, CloneTile(tile));
        }
                     ...
                     ?? new List<OffenseWorldSiteStateData>())
        {
            if (site == null
                || string.IsNullOrWhiteSpace(site.siteId)
                || !tiles.ContainsKey(site.Coord)
                || sites.ContainsKey(site.siteId))
            {
                throw new InvalidOperationException(
                    $"Invalid or duplicate offense site '{site?.siteId ?? "null"}'.");
            }

            sites.Add(site.siteId, CloneSite(site));
        }

        foreach (OffenseUrgentSiteStateData site in saveData.urgentSites
                     ?? new List<OffenseUrgentSiteStateData>())
        {
            if (site == null
                || string.IsNullOrWhiteSpace(site.siteId)
                || !tiles.ContainsKey(site.Coord)
                || urgentSites.ContainsKey(site.siteId))
            {
                throw new InvalidOperationException(
                    $"Invalid or duplicate urgent offense site '{site?.siteId ?? "null"}'.");
            }

            urgentSites.Add(site.siteId, CloneUrgentSite(site));
        }

        nextDynamicSiteSequence = ResolveNextSequence(sites.Keys, "dynamic:");
        nextUrgentSiteSequence = ResolveNextSequence(urgentSites.Keys, "urgent:");
        Changed?.Invoke();
    }
