...
public interface IOffenseReturnSafetyRuntime
{
    ...
    void Restore(IEnumerable<OffenseReturnSafetyStateData> states);
}

public sealed class OffenseReturnSafetyRuntime : IOffenseReturnSafetyRuntime
{
    private readonly IOffenseWorldSimulation world;
    private readonly Dictionary<string, OffenseReturnSafetyStateData> states =
    ...
    public void Restore(IEnumerable<OffenseReturnSafetyStateData> restored)
    {
        states.Clear();
        foreach (OffenseReturnSafetyStateData source in
                 restored ?? Array.Empty<OffenseReturnSafetyStateData>())
        {
            if (source == null || string.IsNullOrWhiteSpace(source.expeditionId))
            {
                continue;
            }

            if (states.ContainsKey(source.expeditionId))
            {
                throw new InvalidOperationException(
                    $"Duplicate offense return-safety state '{source.expeditionId}'.");
            }

            states.Add(source.expeditionId, Clone(source));
        }
    }
        ...
        OffenseReturnSafetyStateData source)
    {
        return new OffenseReturnSafetyStateData
        {
            ...
            safeStepBudget = Mathf.Max(0, source.safeStepBudget),
            protectedForcedCombatCount = Mathf.Max(
                0,
                source.protectedForcedCombatCount),
            nonCombatPitySteps = Mathf.Max(0, source.nonCombatPitySteps)
        };
    }
...
public interface IOffenseTravelRuntime
{
    ...
    void Restore(IEnumerable<OffenseTravelStateData> states);
}
...
public sealed class OffenseTravelRuntime : IOffenseTravelRuntime
{
    ...
    private readonly Dictionary<string, OffenseTravelStateData> states =
    ...
    public void Tick(float deltaTime)
    {
        ...
        foreach (string expeditionId in movingExpeditions)
        {
            ...
            if (state.progressToNextTile < 2.5f)
            {
                continue;
            }

            state.progressToNextTile -= 2.5f;
            TryAdvanceOneStep(
                expeditionId,
    ...
    public void Restore(IEnumerable<OffenseTravelStateData> restored)
    {
        states.Clear();
        foreach (OffenseTravelStateData source in
                 restored ?? Array.Empty<OffenseTravelStateData>())
        {
            if (source == null
                || string.IsNullOrWhiteSpace(source.expeditionId)
                || states.ContainsKey(source.expeditionId)
                || !world.TryGetTile(source.CurrentCoord, out _)
                || !world.TryGetTile(source.DestinationCoord, out _))
            {
                throw new InvalidOperationException(
            ...
            if (clone.remainingPath.Any(coord =>
                    !world.TryGetTile(coord.ToCoord(), out OffenseHexTileState tile)
                    || tile.blocked))
            {
                clone.remainingPath.Clear();
            }

            states.Add(clone.expeditionId, clone);
        }
    }
    ...
    private static OffenseTravelStateData Clone(OffenseTravelStateData source)
    {
        return new OffenseTravelStateData
        {
            ...
            remainingPath = (source.remainingPath
                ...
                .ToList(),
            progressToNextTile = Mathf.Max(0f, source.progressToNextTile),
            exposure = Mathf.Max(0f, source.exposure),
            pausedForDecision = source.pausedForDecision,
            pausedForBattle = source.pausedForBattle,
            eventSequence = Mathf.Max(0, source.eventSequence)
        };
    }
...
public interface IOffenseDecisionRuntime
{
    ...
    void Restore(IEnumerable<OffenseDecisionStateData> states);
}
...
public sealed class OffenseDecisionRuntime : IOffenseDecisionRuntime
{
    ...
    private readonly IOffenseReturnSafetyRuntime returnSafety;
    private readonly Dictionary<string, OffenseDecisionStateData> active =
    ...
    public void Restore(IEnumerable<OffenseDecisionStateData> states)
    {
        active.Clear();
        foreach (OffenseDecisionStateData source in
                 states ?? Array.Empty<OffenseDecisionStateData>())
        {
            if (source == null
                || string.IsNullOrWhiteSpace(source.expeditionId)
                || active.ContainsKey(source.expeditionId)
            ...
            active.Add(source.expeditionId, Clone(source));
        }
    }
