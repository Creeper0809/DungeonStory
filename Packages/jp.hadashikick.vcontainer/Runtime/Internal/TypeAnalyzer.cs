...
namespace VContainer.Internal
{
    ...
    static class TypeAnalyzer
    {
        ...
        public static void CheckCircularDependency(IReadOnlyList<Registration> registrations, Registry registry)
        {
            ...
            // A registration's outgoing dependency graph is immutable while the
            // container is being built. Once that graph has been checked without
            // finding a cycle, traversing it again from every other root is both
            // redundant and catastrophically expensive for a large shared DAG.
            var verified = new HashSet<Registration>();

            for (var i = 0; i < registrations.Count; i++)
            {
                circularDependencyChecker.Clear();
                CheckCircularDependencyRecursive(
                    new DependencyInfo(registrations[i]),
                    registry,
        ...
        static void CheckCircularDependencyRecursive(
            DependencyInfo current,
            Registry registry,
            Stack<DependencyInfo> stack,
            HashSet<Registration> verified)
        {
            if (verified.Contains(current.Dependency))
            {
                return;
            }
            ...
            if (Cache.TryGetValue(current.ImplementationType, out var injectTypeInfo))
            {
                if (injectTypeInfo.InjectConstructor != null)
                {
                    for (var paramIndex = 0; paramIndex < injectTypeInfo.InjectConstructor.ParameterInfos.Length; paramIndex++)
                    {
                        ...
                        {
                            CheckCircularDependencyRecursive(new DependencyInfo(parameterRegistration, current.Dependency, injectTypeInfo.InjectConstructor.ConstructorInfo, x), registry, stack, verified);
                        }
                    }
                }
                ...
                if (injectTypeInfo.InjectMethods != null)
                {
                    foreach (var methodInfo in injectTypeInfo.InjectMethods)
                    {
                        for (var paramIndex = 0; paramIndex < methodInfo.ParameterInfos.Length; paramIndex++)
                        {
                            ...
                            if (registry.TryGet(x.ParameterType, key, out var parameterRegistration))
                            {
                                CheckCircularDependencyRecursive(new DependencyInfo(parameterRegistration, current.Dependency, methodInfo.MethodInfo, x), registry, stack, verified);
                            }
                        }
                    }
                }
                ...
                if (injectTypeInfo.InjectFields != null)
                {
                    foreach (var x in injectTypeInfo.InjectFields)
                    {
                        ...
                        {
                            CheckCircularDependencyRecursive(new DependencyInfo(fieldRegistration, current.Dependency, x.FieldInfo), registry, stack, verified);
                        }
                    }
                }
                ...
                if (injectTypeInfo.InjectProperties != null)
                {
                    foreach (var x in injectTypeInfo.InjectProperties)
                    {
                        ...
                        {
                            CheckCircularDependencyRecursive(new DependencyInfo(propertyRegistration, current.Dependency, x.PropertyInfo), registry, stack, verified);
                        }
                    }
                }
            }
            ...
            stack.Pop();
            verified.Add(current.Dependency);
        }
    }
}
